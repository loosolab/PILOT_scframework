...
def  plot_top_genes_patterns(rank_genes: pd.DataFrame,
    ...
    fig, axes = plt.subplots(10, len(n_clusters), constrained_layout = True,
                             figsize = (len(n_clusters) * 40, 30 * n_px))
...
def plot_hallmark_genes_clusters(adata: ad.AnnData,
    ...
    save_cell_type = adata.uns['gene_selection_heatmap']['cell_type']
    if save_cell_type == cell_type:
        IFrame(path + "gene_selection_analysis/" + str(cell_type) + "/heatmap_culstered.pdf", width="100%", height="100%")
        IFrame(path + "gene_selection_analysis/" + str(cell_type) + "/clusters_activities.pdf", width="100%", height="100%")
        ...
        ans = sns.clustermap(data, 
                     ...
                     cmap='Reds',
                     fmt='',
                     figsize=((len(GO_terms)*0.1)*0.70, len(GO_terms)*0.12),
                     row_cluster=False,
                     dendrogram_ratio=(0.05, 0.05),
                     cbar_pos=(0, 0.45, .01, .2),
